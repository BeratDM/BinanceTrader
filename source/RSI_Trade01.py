...
def calculate_trade(client, closes):
    ...
    if len(closes) > RSI_PERIOD:
        ...
        try:
            ...
            if last_rsi > RSI_OVERBOUGHT or (
                in_position
                and sell_if_up
                and (last_close > price_of_position * sell_if_up_ratio)
            ):
                if last_rsi > RSI_OVERBOUGHT:
                    print("in overbought area.")
                if last_close > price_of_position * sell_if_up_ratio and sell_if_up:
                    print(
                        "current profit is above the percentage of {}".format(
                            sell_if_up_ratio
                        )
                    )
                print(in_position)
                if in_position:
                    ...
                    order_success = order_actions.fill_order(
                        client=client, calculated_order=cook_order(SIDE_SELL)
                    )
                    if order_success:
                        in_position = False
                        price_of_position = 10
            ...
            if last_rsi < RSI_OVERSOLD:
                ...
                if in_position:
                    ...
                    order_success = order_actions.fill_order(
                        client=client, calculated_order=cook_order(SIDE_BUY)
                    )
                    if order_success:
                        in_position = True
